...
/// 
/// <author>
/// Jackson Dunstan, https://jacksondunstan.com/articles/5126
/// </author>
/// 
/// <license>
/// MIT
...
public struct BitArray64 : IEquatable<BitArray64>
{
    ...
    public static implicit operator BitArray64(ulong value) => new BitArray64(value);
    public static implicit operator ulong(BitArray64 array) => array.Bits;
    ...
    {
        Assert.IsTrue(index >= 0 && index < 64,"Index out of bounds: " + index);
    }
    ...
    public int BitCountInSection(int s, bool firstIndex = true)
    {
        if (s < 0 || s > 7)
            return 0;
        ...
        for (int i = 0; i < 8; ++i)
        {
            if (firstIndex ? this[s, i] : this[i, s])
                count++;
        }
